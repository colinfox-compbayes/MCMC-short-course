...
@author: colin
06 october 2022: modify compute4.py to infer from corrupted pixel-wise data.
"""
...
def goodbad_mcmc(N):
    ...
    im = Image.open('corruptedslide.tif')
    # im = Image.open('slide.tif')
    # im.show() # displays as external png
    ...
    MoveRatio = np.array([1, 1, 1, 1, 10], dtype='float32') 
    MoveProb = np.cumsum(MoveRatio[0:-1]/sum(MoveRatio))
    
    showeach = 1000          # subsampling for plotting state

    mean_num = 5.          # my guess at mean number of points
    pgeo = 1/(1+mean_num)   # parameter in geometric distribution prior over number
    pmask = 0.005            # my guess at prod data is crap
    
    bad_data = np.zeros(Dslide.shape,dtype='int32') # intiialize indicator field for bad data as false
    
    X = [bad_data]                  # starting state is bad-data field and null state (state is list of 3-lists)
    #Xlist = [] # a list of list states
    llold = loglike(X,Dslide)
    lpold = logprior(X,pgeo,pmask)
    ...
    for iter in range(0, N):
        ...
        match str(kernum):
            ...
            case '4':       # swap two marked points
                Xp, lh = Simh4(X)
            case '5':       # flip a bad-data mask 
                Xp, lh = Simh5(X)
    
        llnew = loglike(Xp,Dslide)
        #print('llnew',llnew)
        lpnew = logprior(Xp,pgeo,pmask)
        ...
        if (np.random.uniform() < math.exp(lalpha)):
            ...
            #accepted
            # X = Xp.copy()
            X = [xi.copy() for xi in Xp] # deep-ish copy
            llold = llnew
            lpold = lpnew
            
        # accumulate stats
        #print('proposal=',Xp,' state X=',X)
        kvec[iter+1] = len(X) - 1 # first in X list is bad data list
        llvec[iter+1] = llold
        lpvec[iter+1] = lpold
...
def loglike(X,D):
    ...
    ll = (-1/(2*stddev**2))*np.sum(np.multiply(1.-X[0],np.square(makeimage(X,npix) - D)) )
...
def logprior(X,pgeo,pmask):
    """Calculate log prior for state X """
    
    k = len(X)
    nobj = k-1 # number of objects (as X[0] is mask)
    if nobj >= 0:
        lpgeo = nobj*math.log(1-pgeo) + math.log(pgeo) # log of geometric prior over number
        lpmask = np.sum(X[0])*(math.log(pmask)-math.log(1.-pmask))   # log of bad data prior
        lp = lpgeo + lpmask
        # lp = lpgeo
    else:
        lp = -math.inf # should never have negative number of objects
...
def Simh1(X):
    ...
    if np.random.uniform() < 0.5:
        ...
        inew = np.random.randint(1,k+1)      # insert new object before index inew, or at end if inew = k+1
        ...
        if k != 1:
            izap = np.random.randint(1,k)
            del Xp[izap]        # remove marked point (that is not mask)
...
def Simh2(X):
    ...
    #print('Simh2:flip')
    if k != 1:
        iflip = np.random.randint(1,k) # pick an index>0
        tmp = X[iflip].copy()          # new pointer to marked-point list
...
def Simh3(X):
    ...
    if k != 1:
        ...
        imove = np.random.randint(1,k)            # pick an index>0
...
def Simh4(X):
    ...
    if k >= 3:
        imove = np.random.randint(1,k,2)          # pick two indices >0
        tmp = X[imove[0]].copy()
        X[imove[0]] = X[imove[1]].copy()
        X[imove[1]] = tmp                         # and swap marked points
...
def Simh5(X):
    """Simulate kernel 5, return proposed state and log Hastings ratio"""
    # propose a bad/good data flag, uniformly at random
    
    Xp = [xi.copy() for xi in X] # deep-ish copy
    bad_data_tmp = Xp[0].copy()
    xyflip = np.random.randint(0,npix,(2)) # pick a random pixel
    
    if np.random.uniform() < 0.5:
        bad_data_tmp[xyflip[0],xyflip[1]] = 1
    else:
        bad_data_tmp[xyflip[0],xyflip[1]] = 0
...
def makeimage(X,npix):
    ...
    State is length k+1 list 3 vectors representing marked points: [bad_data_mask, (x1,y1,l1), ... , (xk,yk,lk)] 
    k+1 is number of points
    ...
    if  k == 1:
        ...
        for icell in range(k-1,0,-1): # count backwards, ignore mask which is X[0]
            #print('icell=',icell)
            if X[icell][2] == 0:
